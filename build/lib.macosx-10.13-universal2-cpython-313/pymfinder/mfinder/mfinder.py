...
# --- START OF MODIFIED SECTION TO FIX 'imp' MODULE ERROR AND .SO PATH ---
# This block replaces the original 'if version_info >= (2,6,0):' block
# and the definition of swig_import_helper() within it.

def swig_import_helper():
    """
    Helper function to load the SWIG-generated C extension module.
    It tries to load the module from the installed location first (using relative import),
    then falls back to the build directory if running from the source tree.
    """
    modname = '_mfinder'
    # The compiled suffix is specific to your environment (macOS, Python 3.13, universal2)
    compiled_suffix = '.cpython-313-darwin.so'

    # 1. Try direct relative import. This is the standard way for installed packages.
    # If the package is installed in site-packages, 'from . import _mfinder' should work.
    try:
        # Use importlib.import_module with a relative path
        # This is equivalent to 'from . import _mfinder' within the 'pymfinder.mfinder' package
        return importlib.import_module(f".{modname}", package="pymfinder.mfinder")
    except ImportError:
        pass # Continue to try loading from file if relative import fails

    # 2. Fallback to loading from file, specifically looking in the build directory
    # This path is relevant when running scripts or tests directly from the source tree,
    # where the .so file is not adjacent to the .py file but in the build output.
    current_file_dir = os.path.dirname(__file__)
    
    # Calculate the project root directory.
    # From 'pymfinder/mfinder/mfinder.py', go up two directories to 'pymfinder/',
    # then one more up to the project root.
    project_root = os.path.dirname(os.path.dirname(current_file_dir))
    
    # The build directory name is specific to OS and Python version.
    # Using the exact name from your previous successful build output for reliability.
    build_lib_dir_name = "lib.macosx-10.13-universal2-cpython-313" 
    
    # Construct the full path to the compiled module in the build directory
    build_module_path = os.path.join(
        project_root, 'build', build_lib_dir_name, 'pymfinder', 'mfinder', modname + compiled_suffix
    )
    
    if os.path.exists(build_module_path):
        path_to_load = build_module_path
    else:
        # If still not found after trying both standard import and build path, raise an error.
        raise ImportError(
            f"Cannot find compiled module '{modname}{compiled_suffix}'. "
            f"Tried relative import and explicit build path: {build_module_path}. "
            "Please ensure the C extension was built and placed correctly, "
            "or that you are running tests against the installed package."
        )

    # Load the module from the determined file path
    spec = importlib.util.spec_from_file_location(modname, path_to_load)
    if spec is None:
        raise ImportError(f"Could not create module spec for compiled module at {path_to_load}")
...
del version_info # Keep this if it was originally outside the 'if' block
try:
